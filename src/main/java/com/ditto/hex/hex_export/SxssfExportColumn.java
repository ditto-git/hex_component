...
import com.ditto.hex.hex_util.ExCellUtil;
import com.ditto.hex.hex_util.ExFormula;
...
public class SxssfExportColumn extends SxssfExport {
    ...
    protected SxssfExportColumn(InputStream inputStream) {
        try {
            ...
            //初始行(样式,参数)
            this.initRow = this.xssfWorkbook.getSheetAt(styleSheetIndex).getRow(Integer.parseInt(ExThreadLocal.getExTemplate().getHexTemplateCells().get(0).getCellStartRow()));
        } catch (IOException e) {
    ...
    public class SxssfExportColumnWrite extends SxssfExportWrite {
        ...
        public void write() {
            ...
            while (!dataList.isEmpty()) {
                ...
                //解析EX公式
                ExFormula.cellFormulaMatch(ExThreadLocal.getExFormulas(), batch);
                ...
                for (Map<String, Object> data : batch) {
                    ...
                    for (HexTemplateCell hexTemplateCell : ExThreadLocal.getExTemplate().getHexTemplateCells()) {
                        //添加序号
